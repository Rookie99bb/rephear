...
async function findEngagedUsers(): Promise<EngagedUserRow[]> {
  return (await db
    .prepare(
...
async function rankingUpdatesForUser(userId: string, cutoff: string): Promise<DigestRankingUpdate[]> {
  const rows = (await db
    .prepare(
...
export async function getDigestCandidates(): Promise<DigestCandidate[]> {
  const candidates: DigestCandidate[] = [];
  for (const user of await findEngagedUsers()) {
    const cutoff = user.last_digest_sent_at ?? user.created_at;
    const rankings = await rankingUpdatesForUser(user.id, cutoff);
...
export async function markDigestSent(userId: string, sentAt: string): Promise<void> {
  await db.prepare("UPDATE users SET last_digest_sent_at = ? WHERE id = ?").run(sentAt, userId);
}
